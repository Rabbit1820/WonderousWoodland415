...
void ACubeDMIMod::OnOverlapBegin(UPrimitiveComponent* OverlappedComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFronSweep, const FHitResult& SweepResult)
{
	AProj415Character* overlappedActor = Cast<AProj415Character>(OtherActor);
	...
	if (overlappedActor)
	{
		...
		float ranNumA = UKismetMathLibrary::RandomFloatInRange(0.f, .5f);
		float ranNumB = UKismetMathLibrary::RandomFloatInRange(0.6f, 1.f);

		FLinearColor randColor = FLinearColor(ranNumX, ranNumY, ranNumZ, 1.f);
		if (dmiMat)
		{
			dmiMat->SetVectorParameterValue("Color", randColor);
			dmiMat->SetScalarParameterValue("Darkness", ranNumA);
			dmiMat->SetScalarParameterValue("Opacity", ranNumB);
			if (colorP)
			{
